...
class SequentialConvolution(nn.Module):
    def __init__(self, input_size, output_size, kernel_size=3, stride=1, padding=1, bias=True):
        super(SequentialConvolution, self).__init__()

        self.conv1 = nn.Sequential(nn.Conv2d(input_size, output_size, kernel_size=kernel_size, stride=stride, padding=padding, bias=True),
                                   nn.BatchNorm2d(output_size),
                                   nn.PReLU())
        self.conv2 = nn.Sequential(nn.Conv2d(output_size, output_size, kernel_size=kernel_size, stride=stride, padding=padding, bias=True),
                                   nn.BatchNorm2d(output_size),
                                   nn.PReLU())
    def forward(self, x):
...
class TDM_L(nn.Module):
    ...
    def __init__(self, nframes, apha=0.5, belta=0.5):
        ...
        self.compress_3 = SequentialConvolution(self.nframes*64, base_filter, 3, 1, 1)  # 多尺度压缩3*3：h*w*3-->h*w*256

        self.conv1 = SequentialConvolution(base_filter, base_filter, 3, 1, 1)  # 相减之前的嵌入
        self.conv2 = SequentialConvolution(base_filter, base_filter, 3, 1, 1)  # 相减特征的增强
        self.conv3 = SequentialConvolution(base_filter, base_filter, 3, 1, 1)  # 相减池化特征的增强

        self.conv4 = SequentialConvolution(base_filter, self.nframes*64, 3, 1, 1)  # 相加之后的增强
...
from modules.MultiTransformer import MultiLayer
from modules.newblock import EfficientSpatialInformationNetwork
class myNet(nn.Module):
    def __init__(self, nframes):
        ...
        self.res_feat2 = nn.Sequential(*modules_body2)
        self.EfficientSpatialInformationNetwork=EfficientSpatialInformationNetwork(64)
    ...
    def forward(self, x, neigbors):
        ...
        B, C, H, W = x.size()
        fea_x = self.fea0(x)
        fea_x = self.EfficientSpatialInformationNetwork(fea_x)
